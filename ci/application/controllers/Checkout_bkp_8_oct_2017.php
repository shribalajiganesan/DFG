		...
		if($this->cart->total_items() > 0 && count($this->session->userdata('order_data')) >0){
	        ...
	        else
	        {
	        	...
	        	foreach($sess_data['items'] as $item) {
	        		...
	        		$order_data['ordered_on'] 		= date('Y-m-d H:i:s');
	        		$order_data['order_status'] 	= 'order placed';
	        		$order_data['payment_status'] 	= 'success';
	        		$order_data['payment_details'] 	= json_encode($payment_data);
	        		$order_data['item_id'] 			= $item['id'];
	        		$order_data['item_name']		= $item['name'];
	        		$order_data['item_qty'] 		= $item['qty'];
	        		$order_data['total_price'] 		= $item['subtotal'];
	        		$order_data['order_details'] 	= json_encode($item);*/
	        	}
